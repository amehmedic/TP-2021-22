...
Biblioteka::Biblioteka(const Biblioteka &biblioteka)
{
    ...
    while(p!=q)
    {
        ...
        MapaKorisnika.insert(std::pair<int,Student*>((*p->second).broj_indeksa,student));
        p++;
    }
    ...
    while(k!=l)
    {
        Knjiga *knjiga=new Knjiga((*k->second).DajNaslov(), (*k->second).DajAutora(), (*k->second).DajZanr(), (*k->second).DajGodinuIzdavanja());
        MapaKnjiga.insert(std::pair<int,Knjiga*>(k->first,knjiga));
        k++;
    }
}
...
int godina_studija, std::string adresa, std::string telefon)
{
    ...
    MapaKorisnika.insert (std::pair<int,Student*>(broj_indeksa,student));
}
...
std::string Zanr, int GodinaIzdavanja)
{
    ...
    knjiga=new Knjiga(NaslovKnjige, ImePisca, Zanr, GodinaIzdavanja);
    MapaKnjiga.insert (std::pair<int,Knjiga*>(EvidencijskiBroj,knjiga));
}
