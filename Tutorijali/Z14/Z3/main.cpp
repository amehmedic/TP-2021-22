...
public:
	...
	void SacuvajStanje(std::string String)
	{
	    std::ofstream izlazni_tok(String, std::ios::binary);
	...
	void AzurirajIzDatoteke(std::string String)
	{
    	std::ifstream ulazni_tok(String);
    	if(!ulazni_tok)
    	{
    	...
    	while(ulazni_tok)
    	{
    	    ...
    	    std::getline(ulazni_tok, String1);
    	    std::getline(ulazni_tok, String2);
    	    if(ulazni_tok.peek()=='-')
    	    {
    	    ...
    	    RegistrirajUtakmicu(String1.c_str(),String2.c_str(),Golovi1,Golovi2);
    	}
	}
};
...
Liga::Liga(std::string String)
	{
	    ...
	    if(broj_timova>max_broj_timova)
	    {
	        throw std::logic_error("Datoteka sadrzi fatalne greske");
	    }
	    if(!ulazni_tok)
	    {
	        throw std::logic_error("Problemi pri citanju datoteke");
